...
const faqData = [
  {
    q: 'Eloy Chaves é um bom bairro para famílias?',
    a: 'É um dos mais procurados por famílias em Jundiaí: ruas arborizadas e tranquilas, boas escolas a poucos minutos, comércio de bairro no dia a dia e a Serra do Japi e parques por perto.',
  },
  {
    q: 'Quanto custa morar em Eloy Chaves?',
    a: 'O m² para compra fica em torno de R$ 7.500. Casas variam de R$ 600 mil a R$ 2,5 milhões; apartamentos a partir de cerca de R$ 450 mil. Locação de casa parte de aproximadamente R$ 3.000/mês.',
  },
  {
    q: 'Quais condomínios existem em Eloy Chaves?',
    a: 'A região concentra condomínios de casas e loteamentos fechados, além de ruas residenciais abertas. A Marina, nossa especialista do bairro, indica o melhor para o seu perfil.',
  },
  {
    q: 'Eloy Chaves é seguro?',
    a: 'É percebido como um dos bairros mais tranquilos de Jundiaí, com forte presença residencial e ruas movimentadas por moradores. Como em qualquer lugar, varia por trecho — a gente te orienta com honestidade.',
  },
];
...
const guide = [
  {
    num: '1',
    title: 'Como é viver aqui',
    text: 'Eloy Chaves tem ritmo de bairro: gente que se conhece, criança andando de bicicleta, manhã com cheiro de pão na padaria. É residencial de verdade — pouco comércio barulhento, muito verde nas ruas — mas com a cidade a poucos minutos sempre que você precisa.',
  },
  {
    num: '2',
    title: 'Escolas',
    text: 'A região é bem servida de escolas particulares e públicas de boa reputação, a maioria a 5–10 minutos. Famílias com filhos em idade escolar são parte do perfil do bairro — e isso se reflete na rotina tranquila e na procura constante.',
  },
  {
    num: '3',
    title: 'Comércio e serviços',
    text: 'Padarias, mercados de bairro, farmácias, pet shops e serviços do dia a dia estão a poucas quadras. Para compras maiores, o Maxi Shopping e o centro de Jundiaí ficam a cerca de 10 minutos de carro.',
  },
  {
    num: '4',
    title: 'Transporte e acessos',
    text: 'O acesso à Rodovia Anhanguera fica a cerca de 6 minutos, facilitando quem trabalha em Campinas ou na capital. As principais avenidas de Jundiaí conectam o bairro ao centro e à estação em poucos minutos.',
  },
  {
    num: '5',
    title: 'Lazer e natureza',
    text: 'O grande trunfo é a proximidade da Serra do Japi e dos parques da região — trilhas, ar puro e verde a cerca de 10 minutos. No bairro, praças e clubes completam o fim de semana das famílias.',
  },
  {
    num: '6',
    title: 'Segurança e infraestrutura',
    text: 'Infraestrutura consolidada: ruas asfaltadas, iluminação, saneamento. É percebido como um dos bairros mais tranquilos de Jundiaí. A Lotus fala disso com honestidade — cada trecho tem suas características, e a gente te orienta de verdade.',
  },
];
...
const vizinhos = [
  { name: 'Medeiros', note: 'Jundiaí' },
  { name: 'Anhangabaú', note: 'Jundiaí' },
  { name: 'Malota', note: 'Jundiaí' },
  { name: 'Jardim do Lago', note: 'Jundiaí' },
];
...
export default function LotusBairro({
  ...
  // waLink montado exatamente como no renderVals do estático.
  const waLink =
    'https://wa.me/' +
    String(whatsapp ?? '5511926143393') +
    '?text=' +
    encodeURIComponent('Quero ver imóveis em Eloy Chaves, Jundiaí.');
  ...
  return (
    <div ref={rootRef}>
      ...
      <div style={parseStyle('max-width:1200px;margin:0 auto;padding:18px 32px 0;font-size:13px;color:#8aa593;')}>
        <Hoverable as="a" href="/lotus-bairro" target="_top" baseStyle={parseStyle('color:#3f6249;')} hoverStyle={parseStyle('color:#b18a4a')}>Bairros</Hoverable> › <Link href="/lotus-home" style={parseStyle('color:#3f6249;')}>Jundiaí</Link> › <span style={parseStyle('color:#15241c;')}>Eloy Chaves</span>
      </div>
      ...
      <section style={parseStyle('max-width:1200px;margin:16px auto 0;padding:0 32px;')}>
        <div style={parseStyle('position:relative;border-radius:22px;overflow:hidden;height:clamp(320px,42vw,460px);background:#1d3a2c;')}>
          <ImageSlot src="img-20170616-092956-578.jpg" id="bairro-hero" style={parseStyle('position:absolute;inset:0;width:100%;height:100%;')} alt="Foto do bairro — rua arborizada / vista da Serra" />
          ...
          <div style={parseStyle('position:absolute;left:0;right:0;bottom:0;padding:clamp(28px,4vw,48px);')}>
            <div style={parseStyle('font-size:12.5px;font-weight:600;letter-spacing:.18em;text-transform:uppercase;color:#cdab6e;margin-bottom:14px;')}>Guia do bairro · Jundiaí</div>
            <h1 style={parseStyle("font-family:'Fraunces',serif;font-weight:300;font-size:clamp(34px,5.2vw,64px);line-height:1.02;letter-spacing:-.02em;color:#f7f2e8;margin:0 0 14px;")}>Morar em Eloy Chaves, Jundiaí</h1>
            <p style={parseStyle('font-size:clamp(15px,1.6vw,19px);color:rgba(247,242,232,.82);font-weight:300;line-height:1.5;max-width:620px;margin:0;')}>O bairro arborizado e tranquilo de Jundiaí, a 10 minutos da Serra do Japi — preferido de famílias que querem espaço sem abrir mão da cidade ao lado.</p>
          </div>
        </div>
        {/* mini stats */}
        <div style={parseStyle('display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:16px;margin-top:18px;')}>
          <div style={parseStyle('background:#fff;border-radius:14px;padding:20px 22px;box-shadow:0 14px 36px -30px rgba(21,36,28,.4);')}>
            <div style={parseStyle("font-family:'Fraunces',serif;font-size:26px;color:#1d3a2c;")}>R$ 7.500</div>
            <div style={parseStyle('font-size:13px;color:#8aa593;margin-top:3px;')}>preço médio do m² (compra)</div>
          </div>
          <div style={parseStyle('background:#fff;border-radius:14px;padding:20px 22px;box-shadow:0 14px 36px -30px rgba(21,36,28,.4);')}>
            <div style={parseStyle("font-family:'Fraunces',serif;font-size:26px;color:#1d3a2c;")}>Famílias</div>
            <div style={parseStyle('font-size:13px;color:#8aa593;margin-top:3px;')}>perfil predominante</div>
          </div>
          <div style={parseStyle('background:#fff;border-radius:14px;padding:20px 22px;box-shadow:0 14px 36px -30px rgba(21,36,28,.4);')}>
            <div style={parseStyle("font-family:'Fraunces',serif;font-size:26px;color:#1d3a2c;")}>~12 min</div>
            <div style={parseStyle('font-size:13px;color:#8aa593;margin-top:3px;')}>até o centro de Jundiaí</div>
          </div>
          <div style={parseStyle('background:#fff;border-radius:14px;padding:20px 22px;box-shadow:0 14px 36px -30px rgba(21,36,28,.4);')}>
            <div style={parseStyle("font-family:'Fraunces',serif;font-size:26px;color:#1d3a2c;")}>~10 min</div>
            <div style={parseStyle('font-size:13px;color:#8aa593;margin-top:3px;')}>até a Serra do Japi</div>
          </div>
        </div>
      </section>
      ...
      <section style={parseStyle('max-width:1200px;margin:0 auto;padding:72px 32px;display:grid;grid-template-columns:1fr 1.05fr;gap:48px;align-items:start;')}>
        <div>
          ...
          <h2 style={parseStyle("font-family:'Fraunces',serif;font-weight:300;font-size:clamp(26px,3vw,38px);color:#15241c;line-height:1.08;margin:0 0 20px;")}>Eloy Chaves em poucas linhas.</h2>
      ...
      <section style={parseStyle('background:#ece2cf;padding:90px 32px;')}>
        <div style={parseStyle('max-width:1080px;margin:0 auto;')}>
          <div style={parseStyle('text-align:center;max-width:620px;margin:0 auto 56px;')}>
            ...
            <h2 style={parseStyle("font-family:'Fraunces',serif;font-weight:300;font-size:clamp(28px,3.4vw,44px);color:#15241c;margin:0;")}>Como é viver em Eloy Chaves.</h2>
          </div>
      ...
      <section style={parseStyle('background:#15241c;padding:80px 32px;position:relative;overflow:hidden;')}>
        ...
        <div style={parseStyle('max-width:1080px;margin:0 auto;position:relative;')}>
          ...
          <div data-stats="" style={parseStyle('display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:28px;')}>
            <div><div data-count="7500" data-prefix="R$ " data-sep="1" style={parseStyle("font-family:'Fraunces',serif;font-weight:300;font-size:clamp(30px,3.4vw,42px);color:#cdab6e;line-height:1;")}>R$ 7.500</div><div style={parseStyle('font-size:13.5px;color:rgba(247,242,232,.7);margin-top:8px;')}>m² médio · compra</div></div>
            <div><div data-count="32" data-prefix="R$ " style={parseStyle("font-family:'Fraunces',serif;font-weight:300;font-size:clamp(30px,3.4vw,42px);color:#cdab6e;line-height:1;")}>R$ 32</div><div style={parseStyle('font-size:13.5px;color:rgba(247,242,232,.7);margin-top:8px;')}>m² médio · locação</div></div>
            <div><div data-count="8" data-prefix="+" data-suffix="%" style={parseStyle("font-family:'Fraunces',serif;font-weight:300;font-size:clamp(30px,3.4vw,42px);color:#cdab6e;line-height:1;")}>+8%</div><div style={parseStyle('font-size:13.5px;color:rgba(247,242,232,.7);margin-top:8px;')}>valorização · 12 meses</div></div>
            <div><div data-count="68" data-suffix=" dias" style={parseStyle("font-family:'Fraunces',serif;font-weight:300;font-size:clamp(30px,3.4vw,42px);color:#cdab6e;line-height:1;")}>68 dias</div><div style={parseStyle('font-size:13.5px;color:rgba(247,242,232,.7);margin-top:8px;')}>tempo médio de venda</div></div>
          </div>
          <div style={parseStyle('display:flex;flex-wrap:wrap;gap:10px;margin-top:36px;')}>
            ...
            <span style={parseStyle('background:rgba(247,242,232,.1);border:1px solid rgba(247,242,232,.2);color:rgba(247,242,232,.85);font-size:13px;padding:6px 13px;border-radius:30px;')}>Casa térrea</span>
            <span style={parseStyle('background:rgba(247,242,232,.1);border:1px solid rgba(247,242,232,.2);color:rgba(247,242,232,.85);font-size:13px;padding:6px 13px;border-radius:30px;')}>Casa em condomínio</span>
            <span style={parseStyle('background:rgba(247,242,232,.1);border:1px solid rgba(247,242,232,.2);color:rgba(247,242,232,.85);font-size:13px;padding:6px 13px;border-radius:30px;')}>Sobrado</span>
            <span style={parseStyle('background:rgba(247,242,232,.1);border:1px solid rgba(247,242,232,.2);color:rgba(247,242,232,.85);font-size:13px;padding:6px 13px;border-radius:30px;')}>Apartamento 3 dorms</span>
          </div>
      ...
      <section style={parseStyle('background:#f7f2e8;padding:90px 32px;')}>
        <div style={parseStyle('max-width:1200px;margin:0 auto;')}>
          <div style={parseStyle('display:flex;flex-wrap:wrap;align-items:flex-end;justify-content:space-between;gap:20px;margin-bottom:36px;')}>
            <h2 style={parseStyle("font-family:'Fraunces',serif;font-weight:300;font-size:clamp(26px,3vw,40px);color:#15241c;margin:0;")}>Imóveis em Eloy Chaves</h2>
            <Link href="/lotus-busca" style={parseStyle('display:inline-flex;align-items:center;gap:8px;color:#1d3a2c;font-weight:600;font-size:15px;border-bottom:1.5px solid #b18a4a;padding-bottom:3px;')}>Ver todos em Eloy Chaves <span>→</span></Link>
          </div>
          <div style={parseStyle('display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:22px;')}>
            {/* sc-for list=imoveis as=m (4) */}
            {imoveis.map((m, i) => (
              <Hoverable key={i} as="a" href="/lotus-imovel" target="_top" baseStyle={parseStyle('display:flex;flex-direction:column;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 16px 40px -30px rgba(21,36,28,.34);transition:transform .3s ease;')} hoverStyle={parseStyle('transform:translateY(-4px)')}>
                ...
                <div style={parseStyle('padding:18px;')}>
                  ...
                  <div style={parseStyle('font-size:13px;font-weight:600;color:#15241c;margin-top:6px;')}>{m.title}</div>
                  <div style={parseStyle('font-size:12.5px;color:#3f6249;margin-top:3px;')}>{m.specs}</div>
                </div>
              </Hoverable>
            ))}
          </div>
      ...
      <section style={parseStyle('background:#3f6249;padding:80px 32px;position:relative;overflow:hidden;')}>
        ...
        <div style={parseStyle('max-width:980px;margin:0 auto;position:relative;display:grid;grid-template-columns:auto 1fr;gap:36px;align-items:center;')}>
          <div style={parseStyle('width:130px;height:130px;border-radius:50%;background:#1d3a2c;overflow:hidden;position:relative;flex-shrink:0;')}><ImageSlot id="bairro-especialista" style={parseStyle('position:absolute;inset:0;width:100%;height:100%;')} alt="Marina" /></div>
          <div>
            ...
            <h2 style={parseStyle("font-family:'Fraunces',serif;font-weight:300;font-size:clamp(24px,2.8vw,34px);color:#f7f2e8;margin:0 0 10px;line-height:1.1;")}>Marina Tavares conhece Eloy Chaves rua por rua.</h2>
            <p style={parseStyle('font-size:15.5px;color:rgba(247,242,232,.8);font-weight:300;line-height:1.55;margin:0 0 20px;max-width:560px;')}>Especialista em Eloy Chaves há mais de 8 anos. CRECI 000001-F. Ela mora e trabalha na região — sabe qual rua pega sol da manhã, qual escola tem vaga e onde mora a melhor padaria.</p>
            <Hoverable as="a" href={waLink} target="_blank" rel="noopener" baseStyle={parseStyle('display:inline-flex;align-items:center;gap:8px;background:#b18a4a;color:#15241c;font-weight:600;font-size:14.5px;padding:12px 24px;border-radius:40px;transition:background .2s;')} hoverStyle={parseStyle('background:#cdab6e')}>Falar com a Marina <span>→</span></Hoverable>
          </div>
        </div>
      </section>
      ...
      <section style={parseStyle('background:#ece2cf;padding:80px 32px;')}>
        <div style={parseStyle('max-width:880px;margin:0 auto;text-align:center;')}>
          <h2 style={parseStyle("font-family:'Fraunces',serif;font-weight:300;font-size:clamp(26px,3.4vw,42px);color:#15241c;margin:0 0 16px;line-height:1.08;")}>Tem um imóvel em Eloy Chaves? Descubra quanto vale.</h2>
        ...
        <div style={parseStyle('max-width:1200px;margin:0 auto;display:grid;grid-template-columns:1fr 1fr;gap:48px;')}>
          <div>
            ...
            <div style={parseStyle('display:flex;flex-direction:column;')}>
              ...
              {vizinhos.map((v, i) => (
                <Hoverable key={i} as="a" href="/lotus-bairro" target="_top" baseStyle={parseStyle('display:flex;align-items:center;justify-content:space-between;gap:16px;padding:16px 0;border-bottom:1px solid rgba(21,36,28,.1);transition:color .2s;')} hoverStyle={parseStyle('color:#b18a4a')}>
      ...
      <section id="mapa" style={parseStyle('background:#1d3a2c;padding:72px 32px;')}>
        <div style={parseStyle('max-width:1180px;margin:0 auto;')}>
          ...
          <h2 style={parseStyle("font-family:'Fraunces',serif;font-weight:300;font-size:clamp(24px,2.6vw,32px);color:#f7f2e8;margin:0 0 24px;")}>Eloy Chaves no mapa</h2>
          <div style={parseStyle('position:relative;border-radius:18px;overflow:hidden;min-height:380px;background:#e7e4d7;')}>
            <iframe title="Eloy Chaves no mapa" src="https://www.google.com/maps?q=Eloy+Chaves,+Jundia%C3%AD,+SP&z=14&output=embed" loading="lazy" referrerPolicy="no-referrer-when-downgrade" style={parseStyle('position:absolute;inset:0;width:100%;height:100%;border:0;')} allowFullScreen></iframe>
          </div>
        </div>
      </section>
